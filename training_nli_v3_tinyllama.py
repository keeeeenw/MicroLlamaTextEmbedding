...
model_name = sys.argv[1] if len(sys.argv) > 1 else "TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T"
# TODO: retry with 32 by loading the existing checkpoint and skip the large batch dataset
train_batch_size = 8  # The larger you select this, the better the results (usually). But it requires more GPU memory
# max_seq_length = 75
num_epochs = 3
...
output_dir = (
    "output/training_nli_v3_" + model_name_formatted + "-" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
)
...
args = SentenceTransformerTrainingArguments(
    ...
    save_total_limit=20,
    logging_steps=100,
    run_name=f"nli-v3-{model_name_formatted}-{train_batch_size}-{num_epochs}-data-filter-1600",  # Will be used in W&B if `wandb` is installed
