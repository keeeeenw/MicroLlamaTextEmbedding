...
# Load the model
base_model = models.Transformer("TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T", tokenizer_args={'pad_token': '[PAD]'})
...
def filter_long_anchors(example):
    for c in example.keys():
        if isinstance(example[c], str) and len(example[c]) > 1600:
            return False
    return True
...
train_args = SentenceTransformerTrainingArguments(
    f"tmp_trainer-first-10000-{num_epochs}-epoch-batch-{train_batch_size}-tinyllama",
...
model.save_pretrained("tinyllama-1.1b-base-first-10000-nli-stsb-quora-nq")
